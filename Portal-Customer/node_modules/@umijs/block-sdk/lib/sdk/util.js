...
function findSpecifier(importNode, specifier) {
  ...
  try {
    for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
      var _specifier$imported, _s$imported;

      const s = _step3.value;
      if (t().isImportDefaultSpecifier(specifier) && t().isImportDefaultSpecifier(s)) return true; // 这里没判断 specifier 类型，所以 name 会不存在

      if (((_specifier$imported = specifier.imported) === null || _specifier$imported === void 0 ? void 0 : _specifier$imported.name) === ((_s$imported = s.imported) === null || _s$imported === void 0 ? void 0 : _s$imported.name)) {
        var _specifier$local, _s$local;

        if (((_specifier$local = specifier.local) === null || _specifier$local === void 0 ? void 0 : _specifier$local.name) === ((_s$local = s.local) === null || _s$local === void 0 ? void 0 : _s$local.name)) return true;
...
function getIdentifierDeclaration(node, path) {
  ...
  let identifierNode = node; // 处理 HOC 的情况，将 HOC 里的 Identifier 筛选出来

  if (t().isCallExpression(node) && ((_node$arguments = node.arguments) === null || _node$arguments === void 0 ? void 0 : _node$arguments.length) > 0 && node.arguments.some(argument => t().isIdentifier(argument))) {
