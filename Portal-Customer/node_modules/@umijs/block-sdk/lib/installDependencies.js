...
const depsArrayToObject = loc => loc.map(dep => ({
  [dep[0]]: dep[1]
})).reduce((pre, next) => _objectSpread({}, pre, {}, next), {});
/**
 * 安装依赖包
 * - 获取项目路径
 * - 递归获得依赖项。
...
function _installDependencies() {
  _installDependencies = _asyncToGenerator(function* ({
    ...
    const initialValue = (0, _getBlockGenerator.dependenciesConflictCheck)(allBlockDependencies, projectPkg.dependencies, mockDevDependencies, _objectSpread({}, projectPkg.devDependencies, {}, projectPkg.dependencies)); // get conflict dependencies and lack dependencies
