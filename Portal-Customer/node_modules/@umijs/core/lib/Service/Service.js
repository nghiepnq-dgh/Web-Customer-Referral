...
class Service extends _events().EventEmitter {
  ...
  init() {
    ...
    return _asyncToGenerator(function* () {
      ...
      _this.initPresetsAndPlugins(); // collect false configs, then add to this.skipPluginIds
  ...
  initPreset(preset) {
    ...
    this.registerPlugin(preset); // TODO: ...defaultConfigs 考虑要不要支持，可能这个需求可以通过其他渠道实现
...
${name} from ${plugin.path} register failed.`);
        ...
        case _enums.ApplyPluginsType.add:
          ...
          try {
            for (_iterator.s(); !(_step = _iterator.n()).done;) {
              ...
              if (!_this2.isPluginEnable(hook.pluginId)) {
                continue;
              }
          ...
          try {
            for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
              ...
              tModify.tapPromise({
                ...
                var _ref3 = _asyncToGenerator(function* (memo) {
  ...
  runCommand({
    ...
    return _asyncToGenerator(function* () {
      (0, _assert().default)(_this4.stage >= _enums.ServiceStage.init, `service is not initialized.`);
      args._ = args._ || []; // shift the command itself

      if (args._[0] === name) args._.shift();
      const command = typeof _this4.commands[name] === 'string' ? _this4.commands[_this4.commands[name]] : _this4.commands[name];
