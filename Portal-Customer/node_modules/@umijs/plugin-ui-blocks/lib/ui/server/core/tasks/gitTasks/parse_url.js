...
function _getCtx() {
  _getCtx = _asyncToGenerator(function* ({
    args = {},
    api
  }) {
    const url = args.url;
    const config = api.config;
    const ctx = yield (0, _blockSdk().getParsedData)(url, _objectSpread({}, config.block || {}, {}, args));
    debug('getCtx', ctx);

    if (!ctx.isLocal) {
      const blocksTempPath = (0, _blockSdk().makeSureMaterialsTempPathExist)(args.dryRun);
