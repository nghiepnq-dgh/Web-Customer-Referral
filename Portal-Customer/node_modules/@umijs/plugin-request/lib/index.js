  ...
  api.onGenerateFiles(() => {
    ...
    try {
      ...
      api.writeTmpFile({
        ...
        content: requestTemplate.replace(/\/\*FRS\*\/(.+)\/\*FRE\*\//, formatResultStr).replace(/\['data'\]/g, dataField ? `['${dataField}']` : '').replace(/data: T;/, dataField ? `${dataField}: T;` : '').replace(/umi-request/g, winPath((0, _path().dirname)(require.resolve('umi-request/package')))).replace(/@umijs\/use-request/g, winPath((0, _path().dirname)(require.resolve('@umijs/use-request/package')))).replace(`import { ApplyPluginsType, history, plugin } from 'umi';`, `
